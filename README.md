# Spin-Wave Fitting Toolkit

This repository bundles the working scripts, configuration files, and helper assets we currently use in a joint project to fit inelastic neutron-scattering data. The core workflow centres on a Python fitter (`fitting_FeS.py`) that ingests experimental peak positions, optimises the exchange/anisotropy parameters of the spin-wave model, and produces publication-ready diagnostics (CSV summaries and PNG overlays).
...
- `FeS_HKE_plot.py` – Utility script that draws the constant-energy contour `calculate_lambda(H,K,L)=E` in the `(H,0,0)-(K,-2K,0)` plane using the parameters reported in `fit_results.csv`. Configure `TARGET_ENERGY` and `FIXED_L` at the top of the script.
...
- `HK_lambda_*.png` (generated by `FeS_HKE_plot.py`) displays the constant-energy contour in the `(H,0,0)-(K,-2K,0)` plane for the energy and L value specified in that script.
...
- **Drawing constant-energy contours**: edit `TARGET_ENERGY`, `FIXED_L`, and (optionally) the parameter-domain `H_PARAM_RANGE` / `K_PARAM_RANGE` inside `FeS_HKE_plot.py`, then run `python FeS_HKE_plot.py` to generate the closed contour in `(H, K)` space.
